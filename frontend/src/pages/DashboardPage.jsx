    ...
    <div className="bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-3 py-2">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-600">Thống kê và báo cáo</p>
          </div>
          
          <div className="flex items-center gap-1">
            <Tabs value={timeRange} onValueChange={setTimeRange} className="w-auto">
              <TabsList className="h-8">
                <TabsTrigger value="week" className="text-xs px-3 py-1">7 ngày</TabsTrigger>
                <TabsTrigger value="month" className="text-xs px-3 py-1">30 ngày</TabsTrigger>
                <TabsTrigger value="year" className="text-xs px-3 py-1">1 năm</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        </div>
        ...
        <Card className="hover:shadow-lg transition-all duration-300 border-l-4 border-l-emerald-500" data-testid="revenue-card">
          <CardContent className="p-2">
            <div className="flex items-center justify-between mb-1">
              ...
              <Badge variant={growthMetrics.revenueGrowth >= 0 ? "default" : "destructive"} className="text-[10px] px-1 py-0">
                <i className={`fa-solid ${getGrowthIcon(growthMetrics.revenueGrowth)} mr-0.5`} />
                {Math.abs(growthMetrics.revenueGrowth || 0).toFixed(1)}%
              </Badge>
            </div>
            <p className="text-[10px] font-medium text-gray-500 uppercase mb-0.5">Doanh thu hôm nay</p>
            <p className="text-base font-bold text-emerald-600">{formatCurrency(stats.todayRevenue)}</p>
          ...
          <CardContent className="p-2">
            <div className="flex items-center justify-between mb-1">
              <div className="w-7 h-7 bg-blue-100 rounded flex items-center justify-center">
                <i className="fa-solid fa-users text-blue-600 text-xs" />
              </div>
              <Badge variant="default" className="text-[10px] bg-blue-500 px-1 py-0">
                <i className="fa-solid fa-user-plus mr-0.5" />
                +{stats.newCustomers}
              </Badge>
            </div>
            <p className="text-[10px] font-medium text-gray-500 uppercase mb-0.5">Tổng khách hàng</p>
            <p className="text-base font-bold text-blue-600">{formatNumber(stats.totalCustomers)}</p>
            <p className="text-[10px] text-gray-400 mt-0.5">
          ...
          <CardContent className="p-2">
            <div className="flex items-center justify-between mb-1">
              <div className="w-7 h-7 bg-purple-100 rounded flex items-center justify-center">
                <i className="fa-solid fa-calendar-check text-purple-600 text-xs" />
              </div>
            ...
            <p className="text-[10px] font-medium text-gray-500 uppercase mb-0.5">Lịch hẹn hôm nay</p>
            <p className="text-base font-bold text-purple-600">{stats.todayBookings}</p>
            <p className="text-[10px] text-gray-400 mt-0.5">
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              {stats.lowStockProducts > 0 && (
                <Badge variant="destructive" className="text-[10px] px-1 py-0">
                  <i className="fa-solid fa-exclamation-triangle mr-0.5" />
                  {stats.lowStockProducts}
                </Badge>
              )}
            </div>
            <p className="text-[10px] font-medium text-gray-500 uppercase mb-0.5">Dịch vụ & Sản phẩm</p>
            <p className="text-base font-bold text-orange-600">{stats.activeServices + stats.activeProducts}</p>
            <p className="text-[10px] text-gray-400 mt-0.5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        <Card className="bg-gradient-to-br from-green-50 to-emerald-50" data-testid="total-revenue-card">
          <CardContent className="p-2">
            <p className="text-[10px] text-gray-600 mb-0.5">Tổng doanh thu</p>
            <p className="text-sm font-bold text-emerald-700">{formatCurrency(stats.totalRevenue)}</p>
          </CardContent>
        </Card>
        
        <Card className="bg-gradient-to-br from-blue-50 to-indigo-50" data-testid="avg-transaction-card">
          <CardContent className="p-2">
            <p className="text-[10px] text-gray-600 mb-0.5">TB giá trị đơn</p>
            <p className="text-sm font-bold text-blue-700">{formatCurrency(stats.avgTransactionValue)}</p>
          </CardContent>
        </Card>
        
        <Card className="bg-gradient-to-br from-purple-50 to-pink-50" data-testid="top-service-card">
          <CardContent className="p-2">
            <p className="text-[10px] text-gray-600 mb-0.5">Dịch vụ hot nhất</p>
            <p className="text-xs font-bold text-purple-700 truncate">{stats.topService}</p>
          </CardContent>
        </Card>
        
        <Card className="bg-gradient-to-br from-orange-50 to-amber-50" data-testid="top-product-card">
          <CardContent className="p-2">
            <p className="text-[10px] text-gray-600 mb-0.5">Sản phẩm hot nhất</p>
            <p className="text-xs font-bold text-orange-700 truncate">{stats.topProduct}</p>
          </CardContent>
        </Card>
      </div>

      {/* Main Charts Section */}
      <Tabs defaultValue="revenue" className="space-y-2">
        <TabsList className="grid w-full grid-cols-3 h-8">
          <TabsTrigger value="revenue" className="text-xs">Doanh thu</TabsTrigger>
          <TabsTrigger value="performance" className="text-xs">Hiệu suất</TabsTrigger>
          <TabsTrigger value="customers" className="text-xs">Khách hàng</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="revenue" className="space-y-2">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
            ...
            <Card className="lg:col-span-2" data-testid="revenue-chart">
              <CardHeader className="p-3 pb-2">
                <CardTitle className="flex items-center gap-2 text-sm">
                  <i className="fa-solid fa-chart-area text-emerald-600 text-xs" />
                  Biểu đồ doanh thu
                </CardTitle>
                <CardDescription className="text-[10px]">Theo dõi doanh thu và mục tiêu theo thời gian</CardDescription>
              </CardHeader>
            ...
            <Card data-testid="category-chart">
              <CardHeader className="p-3 pb-2">
                ...
                <CardDescription className="text-[10px]">Theo danh mục</CardDescription>
              </CardHeader>
              <CardContent className="p-3 pt-0">
          ...
          <Card data-testid="hourly-revenue-chart">
            <CardHeader className="p-3 pb-2">
              <CardTitle className="flex items-center gap-2 text-sm">
                <i className="fa-solid fa-clock text-blue-600 text-xs" />
                Doanh thu theo giờ (Hôm nay)
              </CardTitle>
              <CardDescription className="text-[10px]">Phân tích doanh thu trong ngày</CardDescription>
            </CardHeader>
            <CardContent className="p-3 pt-0">
        ...
        <TabsContent value="performance" className="space-y-2">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
            ...
            <Card data-testid="services-chart">
              <CardHeader className="p-3 pb-2">
                ...
                <CardDescription className="text-[10px]">Top 5 dịch vụ được sử dụng nhiều nhất</CardDescription>
              </CardHeader>
              <CardContent className="p-3 pt-0">
                <ResponsiveContainer width="100%" height={220}>
            ...
            <Card data-testid="products-chart">
              <CardHeader className="p-3 pb-2">
                <CardTitle className="flex items-center gap-2 text-sm">
                  <i className="fa-solid fa-box-open text-orange-600 text-xs" />
                  Sản phẩm bán chạy
                </CardTitle>
                <CardDescription className="text-[10px]">Top 5 sản phẩm được mua nhiều nhất</CardDescription>
              </CardHeader>
              <CardContent className="p-3 pt-0">
                <ResponsiveContainer width="100%" height={220}>
          ...
          <Card data-testid="staff-performance">
            <CardHeader className="p-3 pb-2">
              <CardTitle className="flex items-center gap-2 text-sm">
                <i className="fa-solid fa-user-tie text-indigo-600 text-xs" />
                Hiệu suất nhân viên
              </CardTitle>
              <CardDescription className="text-[10px]">Đánh giá năng suất làm việc</CardDescription>
            </CardHeader>
            <CardContent className="p-3 pt-0">
                    ...
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 text-[10px] text-gray-500">
                          <span><i className="fa-solid fa-scissors mr-0.5" />{staff.services} dịch vụ</span>
                          <span><i className="fa-solid fa-star mr-0.5 text-yellow-500" />{staff.rating}</span>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-emerald-600 text-sm">{formatCurrency(staff.revenue)}</p>
                      <p className="text-[10px] text-gray-500">Doanh thu</p>
                    </div>
                  </div>
                )) : (
                  <div className="text-center py-6 text-gray-500">
                    <i className="fa-solid fa-user-tie text-3xl mb-2 text-gray-300" />
                    <p className="text-xs">Chưa có dữ liệu nhân viên</p>
                  </div>
                )}
              </div>
            </CardContent>
        ...
        <TabsContent value="customers" className="space-y-2">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
            ...
            <Card data-testid="top-customers">
              <CardHeader className="p-3 pb-2">
                <CardTitle className="flex items-center gap-2 text-sm">
                  <i className="fa-solid fa-crown text-yellow-500 text-xs" />
                  Khách hàng VIP
                </CardTitle>
                <CardDescription className="text-[10px]">Top khách hàng chi tiêu cao nhất</CardDescription>
              </CardHeader>
              <CardContent className="p-3 pt-0">
                <div className="space-y-2">
                  {topCustomers.map((customer, index) => (
                    <div key={index} className="flex items-center justify-between p-2 bg-gradient-to-r from-gray-50 to-white rounded hover:shadow transition-all">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-full flex items-center justify-center text-white font-bold text-xs">
                          {index + 1}
                        </div>
                        <div>
                          <p className="font-semibold text-gray-900 text-xs">{customer.name}</p>
                          <p className="text-[10px] text-gray-500">{customer.phone}</p>
                          <p className="text-[10px] text-gray-500">
                            <i className="fa-solid fa-repeat mr-0.5" />
                            {customer.visits} lượt
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-emerald-600 text-xs">{formatCurrency(customer.total)}</p>
            ...
            <Card data-testid="customer-segments">
              <CardHeader className="p-3 pb-2">
                <CardTitle className="flex items-center gap-2 text-sm">
                  <i className="fa-solid fa-users-gear text-blue-600 text-xs" />
                  Phân khúc khách hàng
                </CardTitle>
                <CardDescription className="text-[10px]">Phân loại theo hành vi mua hàng</CardDescription>
              </CardHeader>
              <CardContent className="p-3 pt-0">
                  ...
                  {customerSegments.map((segment, index) => (
                    <div key={index}>
                      <div className="flex items-center justify-between mb-0.5 text-xs">
                        <div className="flex items-center gap-1.5">
                          <div className="w-2 h-2 rounded" style={{ backgroundColor: segment.color }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
        ...
        <Card data-testid="today-bookings">
          <CardHeader className="p-3 pb-2">
            <CardTitle className="flex items-center gap-2 text-sm">
              <i className="fa-solid fa-calendar-day text-purple-600 text-xs" />
              Lịch hẹn hôm nay
            </CardTitle>
            <CardDescription className="text-[10px]">Danh sách lịch hẹn trong ngày</CardDescription>
          </CardHeader>
          <CardContent className="p-3 pt-0">
            {todayBookings.length > 0 ? (
              <div className="space-y-1.5">
                {todayBookings.map((booking, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded hover:bg-gray-100 transition-colors border-l-4 border-l-purple-500">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center">
                        <i className="fa-solid fa-user text-purple-600 text-xs" />
                      </div>
                      <div>
                        <p className="font-medium text-gray-900 text-xs">{booking.customerName || 'Khách hàng'}</p>
                        <p className="text-[10px] text-gray-500">{booking.service || 'Dịch vụ'}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold text-gray-900 text-xs">{booking.time || '00:00'}</p>
                      <Badge 
                        variant={booking.status === 'completed' ? 'default' : booking.status === 'canceled' ? 'destructive' : 'secondary'}
                        className="text-[10px] mt-0.5 px-1 py-0"
                      >
              ...
              <div className="text-center py-8 text-gray-500">
                <i className="fa-solid fa-calendar-xmark text-3xl mb-2 text-gray-300" />
                <p className="font-medium text-xs">Không có lịch hẹn nào hôm nay</p>
                <p className="text-[10px] mt-0.5">Các lịch hẹn mới sẽ hiển thị tại đây</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card data-testid="recent-transactions">
          <CardHeader className="p-3 pb-2">
            <CardTitle className="flex items-center gap-2 text-sm">
              <i className="fa-solid fa-receipt text-emerald-600 text-xs" />
              Giao dịch gần đây
            </CardTitle>
            <CardDescription className="text-[10px]">Các giao dịch mới nhất</CardDescription>
          </CardHeader>
          <CardContent className="p-3 pt-0">
            {recentTransactions.length > 0 ? (
              <div className="space-y-1.5">
                {recentTransactions.map((transaction, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded hover:bg-gray-100 transition-colors border-l-4 border-l-emerald-500">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-emerald-100 rounded-full flex items-center justify-center">
                        <i className="fa-solid fa-money-bill-wave text-emerald-600 text-xs" />
                      </div>
                      <div>
                        <p className="font-medium text-gray-900 text-xs">{transaction.customerName}</p>
                        <p className="text-[10px] text-gray-500">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-emerald-600 text-xs">{formatCurrency(transaction.total)}</p>
              ...
              <div className="text-center py-8 text-gray-500">
                <i className="fa-solid fa-receipt text-3xl mb-2 text-gray-300" />
                <p className="font-medium text-xs">Chưa có giao dịch nào</p>
                <p className="text-[10px] mt-0.5">Giao dịch mới sẽ hiển thị tại đây</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <Card data-testid="performance-indicators">
        <CardHeader className="p-3 pb-2">
          <CardTitle className="flex items-center gap-2 text-sm">
            <i className="fa-solid fa-gauge-high text-indigo-600 text-xs" />
            Chỉ số hiệu suất kinh doanh
          </CardTitle>
          <CardDescription className="text-[10px]">Đánh giá các chỉ số quan trọng</CardDescription>
        </CardHeader>
        <CardContent className="p-3 pt-0">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-gray-600">Tỷ lệ hoàn thành booking</span>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-gray-600">Mục tiêu doanh thu tháng</span>
                <span className="text-xs font-bold text-gray-900">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-gray-600">Khách hàng quay lại</span>
                <span className="text-xs font-bold text-gray-900">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-gray-600">Tỷ lệ hủy booking</span>
      ...
      <Card className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50" data-testid="growth-metrics">
        <CardHeader className="p-3 pb-2">
          <CardTitle className="flex items-center gap-2 text-sm">
            <i className="fa-solid fa-chart-line text-indigo-600 text-xs" />
        ...
        <CardContent className="p-3 pt-0">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <div className="text-center p-2 bg-white rounded">
              <i className={`fa-solid ${getGrowthIcon(growthMetrics.monthRevenueGrowth)} text-lg mb-1 ${getGrowthColor(growthMetrics.monthRevenueGrowth)}`} />
              <p className="text-[10px] text-gray-600 mb-0.5">Doanh thu tháng</p>
              <p className={`text-base font-bold ${getGrowthColor(growthMetrics.monthRevenueGrowth)}`}>
                {growthMetrics.monthRevenueGrowth >= 0 ? '+' : ''}{(growthMetrics.monthRevenueGrowth || 0).toFixed(1)}%
              </p>
            </div>
            
            <div className="text-center p-2 bg-white rounded">
              <i className="fa-solid fa-arrow-trend-up text-lg mb-1 text-blue-600" />
              <p className="text-[10px] text-gray-600 mb-0.5">Khách hàng mới</p>
              <p className="text-base font-bold text-blue-600">+{(growthMetrics.customerGrowth || 0).toFixed(1)}%</p>
            </div>
            
            <div className="text-center p-2 bg-white rounded">
              <i className="fa-solid fa-arrow-trend-up text-lg mb-1 text-purple-600" />
              <p className="text-[10px] text-gray-600 mb-0.5">Booking</p>
              <p className="text-base font-bold text-purple-600">+{(growthMetrics.bookingGrowth || 0).toFixed(1)}%</p>
            </div>
            
            <div className="text-center p-2 bg-white rounded">
              <i className="fa-solid fa-arrow-trend-up text-lg mb-1 text-orange-600" />
              <p className="text-[10px] text-gray-600 mb-0.5">Giá trị TB đơn hàng</p>
              <p className="text-base font-bold text-orange-600">+12.4%</p>
            </div>
          </div>
        </CardContent>
      </Card>
