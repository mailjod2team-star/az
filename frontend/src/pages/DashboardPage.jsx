...
const DashboardPage = () => {
  ...
  const loadDashboardData = useCallback(() => {
    ...
    try {
      ...
      setGrowthMetrics({
        ...
        monthRevenueGrowth: monthRevenueGrowth,
      ...
      // Staff performance (mock data for now)
      setStaffPerformance([
        { name: 'Nguyễn Văn A', services: 25, revenue: 15000000, rating: 4.8 },
        { name: 'Trần Thị B', services: 22, revenue: 13500000, rating: 4.7 },
        { name: 'Lê Văn C', services: 20, revenue: 12000000, rating: 4.6 },
        { name: 'Phạm Thị D', services: 18, revenue: 10500000, rating: 4.5 },
      ]);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Cập nhật lần cuối: {format(lastUpdate, 'HH:mm:ss dd/MM/yyyy')}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <i className={`fa-solid fa-rotate-right mr-2 ${isLoading ? 'animate-spin' : ''}`} />
            Làm mới
          </Button>
      ...
      {/* Key Metrics Cards - Compact & Modern */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <Card className="hover:shadow-xl transition-all duration-300 border-l-4 border-l-emerald-500" data-testid="revenue-card">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center">
                <i className="fa-solid fa-money-bill-trend-up text-emerald-600 text-lg" />
              </div>
              <Badge variant={growthMetrics.revenueGrowth >= 0 ? "default" : "destructive"} className="text-xs">
                <i className={`fa-solid ${getGrowthIcon(growthMetrics.revenueGrowth)} mr-1`} />
                {Math.abs(growthMetrics.revenueGrowth || 0).toFixed(1)}%
              </Badge>
            </div>
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">Doanh thu hôm nay</p>
            <p className="text-xl md:text-2xl font-bold text-emerald-600">{formatCurrency(stats.todayRevenue)}</p>
        ...
        <Card className="hover:shadow-xl transition-all duration-300 border-l-4 border-l-blue-500" data-testid="customers-card">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center">
                <i className="fa-solid fa-users text-blue-600 text-lg" />
              </div>
            ...
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">Tổng khách hàng</p>
            <p className="text-xl md:text-2xl font-bold text-blue-600">{formatNumber(stats.totalCustomers)}</p>
            <p className="text-xs text-gray-400 mt-1">
        ...
        <Card className="hover:shadow-xl transition-all duration-300 border-l-4 border-l-purple-500" data-testid="bookings-card">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center">
                <i className="fa-solid fa-calendar-check text-purple-600 text-lg" />
              </div>
            ...
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">Lịch hẹn hôm nay</p>
            <p className="text-xl md:text-2xl font-bold text-purple-600">{stats.todayBookings}</p>
            <p className="text-xs text-gray-400 mt-1">
        ...
        <Card className="hover:shadow-xl transition-all duration-300 border-l-4 border-l-orange-500" data-testid="products-services-card">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              {stats.lowStockProducts > 0 && (
                <Badge variant="destructive" className="text-xs">
                  ...
                  {stats.lowStockProducts} thấp
                </Badge>
              )}
            </div>
            <p className="text-xs font-medium text-gray-500 uppercase mb-1">Dịch vụ & Sản phẩm</p>
            <p className="text-xl md:text-2xl font-bold text-orange-600">{stats.activeServices + stats.activeProducts}</p>
            <p className="text-xs text-gray-400 mt-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card className="bg-gradient-to-br from-green-50 to-emerald-50" data-testid="total-revenue-card">
          <CardContent className="p-3">
            ...
            <p className="text-lg font-bold text-emerald-700">{formatCurrency(stats.totalRevenue)}</p>
          </CardContent>
        </Card>
        
        <Card className="bg-gradient-to-br from-blue-50 to-indigo-50" data-testid="avg-transaction-card">
          <CardContent className="p-3">
            <p className="text-xs text-gray-600 mb-1">TB giá trị đơn</p>
            <p className="text-lg font-bold text-blue-700">{formatCurrency(stats.avgTransactionValue)}</p>
          </CardContent>
        </Card>
        
        <Card className="bg-gradient-to-br from-purple-50 to-pink-50" data-testid="top-service-card">
          <CardContent className="p-3">
            <p className="text-xs text-gray-600 mb-1">Dịch vụ hot nhất</p>
        ...
        <Card className="bg-gradient-to-br from-orange-50 to-amber-50" data-testid="top-product-card">
          <CardContent className="p-3">
            <p className="text-xs text-gray-600 mb-1">Sản phẩm hot nhất</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-white rounded-lg">
              ...
              <p className={`text-2xl font-bold ${getGrowthColor(growthMetrics.monthRevenueGrowth)}`}>
                {growthMetrics.monthRevenueGrowth >= 0 ? '+' : ''}{growthMetrics.monthRevenueGrowth.toFixed(1)}%
              </p>
            </div>
            ...
            <div className="text-center p-4 bg-white rounded-lg">
              ...
              <p className="text-2xl font-bold text-blue-600">+{growthMetrics.customerGrowth.toFixed(1)}%</p>
            </div>
            
            <div className="text-center p-4 bg-white rounded-lg">
              <i className="fa-solid fa-arrow-trend-up text-2xl mb-2 text-purple-600" />
              <p className="text-xs text-gray-600 mb-1">Booking</p>
              <p className="text-2xl font-bold text-purple-600">+{growthMetrics.bookingGrowth.toFixed(1)}%</p>
            </div>
