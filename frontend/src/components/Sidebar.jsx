...
export const Sidebar = ({ activeView, onNavigate }) => {
  const menuItems = [
    ...
    { id: 'staff', icon: 'fa-user-tie', label: 'Nhân viên', color: 'indigo' },
    { id: 'commission', icon: 'fa-hand-holding-dollar', label: 'Lương & Hoa hồng', color: 'cyan' },
    ...
    { id: 'feedback', icon: 'fa-comment-dots', label: 'Đánh giá', color: 'yellow' },
    { id: 'notifications', icon: 'fa-bell', label: 'Thông báo', color: 'orange' },
    { id: 'settings', icon: 'fa-cog', label: 'Cài đặt', color: 'gray' },
  ];

  return (
    <motion.div
      className="w-52 bg-gradient-to-br from-emerald-50 via-white to-teal-50 backdrop-blur-sm border-r-2 border-gray-300 h-screen flex flex-col shadow-sm relative"
      initial={{ x: -300 }}
      animate={{ x: 0 }}
    ...
    >
      <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar">
        <nav className="p-2 space-y-0.5">
          ...
          {/* Divider - Nhân sự */}
          <div className="pt-3 pb-2 px-2">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Nhân sự</p>
          </div>
          ...
          {menuItems.slice(12, 15).map((item) => (
            <motion.button
              key={item.id}
              onClick={() => onNavigate(item.id)}
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg transition-all font-medium text-sm ${
      ...
      <div className="p-3 border-t border-gray-200 bg-white/50">
        <div className="text-xs text-gray-500 text-center">
          <p className="font-semibold">DEAR POS v1.0</p>
        </div>
      </div>
