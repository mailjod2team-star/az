...
export const SOCDashboard = () => {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-background p-6" data-testid="soc-dashboard">
      {/* Overview Stats */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          <motion.div
            ...
            className={`p-6 rounded-lg border ${getStatusBg(stat.status)}`}
          >
            <div className="flex items-center justify-between mb-3">
              <i className={`fa-solid ${stat.icon} text-2xl ${getStatusColor(stat.status)}`} />
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {serverStatusData.map((server, index) => (
            <div key={index} className={`p-4 rounded-lg border ${getStatusBg(server.status)}`}>
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-semibold text-foreground">{server.name}</h4>
                <span className={`px-2 py-1 rounded text-xs font-semibold ${getStatusColor(server.status)}`}>
          ...
          {databaseMetrics.map((metric, index) => (
            <div key={index} className={`p-4 rounded-lg border ${getStatusBg(metric.status)}`}>
              ...
              <div className="w-full bg-gray-700 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${metric.status === 'healthy' ? 'bg-green-500' : 'bg-yellow-500'}`}
                  style={{ width: `${(metric.current / metric.max) * 100}%` }}
                />
              </div>
              <div className="text-xs text-muted-foreground mt-1">Max: {metric.max}{metric.unit || ''}</div>
            </div>
          ))}
        </div>
      </motion.div>
      ...
      >
        <h3 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground ml-auto">Interactive Topology View</span>
        </h3>
        
        {/* Network Map Visualization */}
        <div className="relative bg-gradient-to-br from-emerald-950/30 via-teal-950/20 to-slate-950/40 rounded-lg p-8 h-[600px] overflow-hidden border border-primary/20">
          {/* Grid Background */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        >
          <h3 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
            <i className="fa-solid fa-laptop text-primary" />
            ...
            {activeDevices.map((device, index) => (
              <div key={index} className="p-3 rounded-lg bg-muted/50 border border-border">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-semibold ${device.risk === 'low' ? 'bg-green-500/20 text-green-500' : 'bg-yellow-500/20 text-yellow-500'}`}>
        ...
        >
          <h3 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
            <i className="fa-solid fa-triangle-exclamation text-primary" />
            Cảnh báo bảo mật
          </h3>
          <div className="space-y-3">
            {securityAlerts.map((alert, index) => (
              <div key={index} className="p-3 rounded-lg bg-muted/50 border border-border">
                ...
                <div className="text-sm text-muted-foreground">
                  Source: <span className="font-mono text-foreground">{alert.source}</span>
                </div>
                <div className="mt-2">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <h4 className="text-sm font-semibold text-muted-foreground mb-3">Cơ sở hạ tầng</h4>
          ...
          <div>
            <h4 className="text-sm font-semibold text-muted-foreground mb-3">Bảo mật</h4>
            <ul className="space-y-2 text-sm">
              <li className="flex justify-between">
          ...
          <div>
            <h4 className="text-sm font-semibold text-muted-foreground mb-3">Hiệu suất</h4>
            <ul className="space-y-2 text-sm">
              <li className="flex justify-between">
